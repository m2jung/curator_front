...
export default function SignUpView(props) {
  ...
  //function
  //email
  const onClickEmailCheck = useCallback((e) => {
...
const onClickTokenCheck = useCallback((e) => {
  if(token === curr_cookie.get('emailCheckToken')) {
...
const onChangePassword = useCallback((e) => {
 ...
 if (!validatePw(currPw)) {
    setPwMsg("영문, 숫자, 특수기호 조합으로 10자리 이상 입력해주세요.")} 
else { setPwMsg("") }
  })
...
const onChangePasswordCheck = useCallback((e) => {
 ...
 if (!validatePw(currPwC)) {
    setPwMsgCheck("영문, 숫자, 특수기호 조합으로 10자리 이상 입력해주세요.")} 
 else { 
